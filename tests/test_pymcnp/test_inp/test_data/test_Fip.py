import pymcnp
from .... import consts
from .... import classes


class Test_Fip:
    class Test_Init(classes.Test_Init):
        element = pymcnp.inp.data.Fip
        EXAMPLES_VALID = [
            {
                'suffix': pymcnp.types.Integer(5),
                'designator': consts.ast.type.DESIGNATOR,
                'x1': consts.ast.type.REAL,
                'y1': consts.ast.type.REAL,
                'z1': consts.ast.type.REAL,
                'ro': pymcnp.types.Real(0),
                'x2': consts.ast.type.REAL,
                'y2': consts.ast.type.REAL,
                'z2': consts.ast.type.REAL,
                'f1': consts.ast.type.REAL,
                'f2': consts.ast.type.REAL,
                'f3': consts.ast.type.REAL,
            }
        ]
        EXAMPLES_INVALID = [
            {
                'suffix': None,
                'designator': consts.ast.type.DESIGNATOR,
                'x1': consts.ast.type.REAL,
                'y1': consts.ast.type.REAL,
                'z1': consts.ast.type.REAL,
                'ro': pymcnp.types.Real(0),
                'x2': consts.ast.type.REAL,
                'y2': consts.ast.type.REAL,
                'z2': consts.ast.type.REAL,
                'f1': consts.ast.type.REAL,
                'f2': consts.ast.type.REAL,
                'f3': consts.ast.type.REAL,
            },
            {
                'suffix': pymcnp.types.Integer(5),
                'designator': None,
                'x1': consts.ast.type.REAL,
                'y1': consts.ast.type.REAL,
                'z1': consts.ast.type.REAL,
                'ro': pymcnp.types.Real(0),
                'x2': consts.ast.type.REAL,
                'y2': consts.ast.type.REAL,
                'z2': consts.ast.type.REAL,
                'f1': consts.ast.type.REAL,
                'f2': consts.ast.type.REAL,
                'f3': consts.ast.type.REAL,
            },
            {
                'suffix': pymcnp.types.Integer(5),
                'designator': consts.ast.type.DESIGNATOR,
                'x1': None,
                'y1': consts.ast.type.REAL,
                'z1': consts.ast.type.REAL,
                'ro': pymcnp.types.Real(0),
                'x2': consts.ast.type.REAL,
                'y2': consts.ast.type.REAL,
                'z2': consts.ast.type.REAL,
                'f1': consts.ast.type.REAL,
                'f2': consts.ast.type.REAL,
                'f3': consts.ast.type.REAL,
            },
            {
                'suffix': pymcnp.types.Integer(5),
                'designator': consts.ast.type.DESIGNATOR,
                'x1': consts.ast.type.REAL,
                'y1': None,
                'z1': consts.ast.type.REAL,
                'ro': pymcnp.types.Real(0),
                'x2': consts.ast.type.REAL,
                'y2': consts.ast.type.REAL,
                'z2': consts.ast.type.REAL,
                'f1': consts.ast.type.REAL,
                'f2': consts.ast.type.REAL,
                'f3': consts.ast.type.REAL,
            },
            {
                'suffix': pymcnp.types.Integer(5),
                'designator': consts.ast.type.DESIGNATOR,
                'x1': consts.ast.type.REAL,
                'y1': consts.ast.type.REAL,
                'z1': None,
                'ro': pymcnp.types.Real(0),
                'x2': consts.ast.type.REAL,
                'y2': consts.ast.type.REAL,
                'z2': consts.ast.type.REAL,
                'f1': consts.ast.type.REAL,
                'f2': consts.ast.type.REAL,
                'f3': consts.ast.type.REAL,
            },
            {
                'suffix': pymcnp.types.Integer(5),
                'designator': consts.ast.type.DESIGNATOR,
                'x1': consts.ast.type.REAL,
                'y1': consts.ast.type.REAL,
                'z1': consts.ast.type.REAL,
                'ro': None,
                'x2': consts.ast.type.REAL,
                'y2': consts.ast.type.REAL,
                'z2': consts.ast.type.REAL,
                'f1': consts.ast.type.REAL,
                'f2': consts.ast.type.REAL,
                'f3': consts.ast.type.REAL,
            },
            {
                'suffix': pymcnp.types.Integer(5),
                'designator': consts.ast.type.DESIGNATOR,
                'x1': consts.ast.type.REAL,
                'y1': consts.ast.type.REAL,
                'z1': consts.ast.type.REAL,
                'ro': pymcnp.types.Real(0),
                'x2': None,
                'y2': consts.ast.type.REAL,
                'z2': consts.ast.type.REAL,
                'f1': consts.ast.type.REAL,
                'f2': consts.ast.type.REAL,
                'f3': consts.ast.type.REAL,
            },
            {
                'suffix': pymcnp.types.Integer(5),
                'designator': consts.ast.type.DESIGNATOR,
                'x1': consts.ast.type.REAL,
                'y1': consts.ast.type.REAL,
                'z1': consts.ast.type.REAL,
                'ro': pymcnp.types.Real(0),
                'x2': consts.ast.type.REAL,
                'y2': None,
                'z2': consts.ast.type.REAL,
                'f1': consts.ast.type.REAL,
                'f2': consts.ast.type.REAL,
                'f3': consts.ast.type.REAL,
            },
            {
                'suffix': pymcnp.types.Integer(5),
                'designator': consts.ast.type.DESIGNATOR,
                'x1': consts.ast.type.REAL,
                'y1': consts.ast.type.REAL,
                'z1': consts.ast.type.REAL,
                'ro': pymcnp.types.Real(0),
                'x2': consts.ast.type.REAL,
                'y2': consts.ast.type.REAL,
                'z2': None,
                'f1': consts.ast.type.REAL,
                'f2': consts.ast.type.REAL,
                'f3': consts.ast.type.REAL,
            },
            {
                'suffix': pymcnp.types.Integer(5),
                'designator': consts.ast.type.DESIGNATOR,
                'x1': consts.ast.type.REAL,
                'y1': consts.ast.type.REAL,
                'z1': consts.ast.type.REAL,
                'ro': pymcnp.types.Real(0),
                'x2': consts.ast.type.REAL,
                'y2': consts.ast.type.REAL,
                'z2': consts.ast.type.REAL,
                'f1': None,
                'f2': consts.ast.type.REAL,
                'f3': consts.ast.type.REAL,
            },
            {
                'suffix': pymcnp.types.Integer(5),
                'designator': consts.ast.type.DESIGNATOR,
                'x1': consts.ast.type.REAL,
                'y1': consts.ast.type.REAL,
                'z1': consts.ast.type.REAL,
                'ro': pymcnp.types.Real(0),
                'x2': consts.ast.type.REAL,
                'y2': consts.ast.type.REAL,
                'z2': consts.ast.type.REAL,
                'f1': consts.ast.type.REAL,
                'f2': None,
                'f3': consts.ast.type.REAL,
            },
            {
                'suffix': pymcnp.types.Integer(5),
                'designator': consts.ast.type.DESIGNATOR,
                'x1': consts.ast.type.REAL,
                'y1': consts.ast.type.REAL,
                'z1': consts.ast.type.REAL,
                'ro': pymcnp.types.Real(0),
                'x2': consts.ast.type.REAL,
                'y2': consts.ast.type.REAL,
                'z2': consts.ast.type.REAL,
                'f1': consts.ast.type.REAL,
                'f2': consts.ast.type.REAL,
                'f3': None,
            },
        ]

    class Test_Mcnp(classes.Test_Mcnp):
        element = pymcnp.inp.data.Fip
        EXAMPLES_VALID = [consts.string.inp.data.FIP]
        EXAMPLES_INVALID = ['hello']


class Test_FipBuilder:
    class TestBuild(classes.Test_Build):
        element = pymcnp.inp.data.FipBuilder
        EXAMPLES_VALID = [
            {
                'suffix': '5',
                'designator': consts.string.type.DESIGNATOR,
                'x1': consts.string.type.REAL,
                'y1': consts.string.type.REAL,
                'z1': consts.string.type.REAL,
                'ro': '0',
                'x2': consts.string.type.REAL,
                'y2': consts.string.type.REAL,
                'z2': consts.string.type.REAL,
                'f1': consts.string.type.REAL,
                'f2': consts.string.type.REAL,
                'f3': consts.string.type.REAL,
            },
            {'suffix': 5, 'designator': consts.string.type.DESIGNATOR, 'x1': 3.1, 'y1': 3.1, 'z1': 3.1, 'ro': 0, 'x2': 3.1, 'y2': 3.1, 'z2': 3.1, 'f1': 3.1, 'f2': 3.1, 'f3': 3.1},
            {
                'suffix': pymcnp.types.Integer(5),
                'designator': consts.ast.type.DESIGNATOR,
                'x1': consts.ast.type.REAL,
                'y1': consts.ast.type.REAL,
                'z1': consts.ast.type.REAL,
                'ro': pymcnp.types.Real(0),
                'x2': consts.ast.type.REAL,
                'y2': consts.ast.type.REAL,
                'z2': consts.ast.type.REAL,
                'f1': consts.ast.type.REAL,
                'f2': consts.ast.type.REAL,
                'f3': consts.ast.type.REAL,
            },
        ]
        EXAMPLES_INVALID = [
            {
                'suffix': None,
                'designator': consts.string.type.DESIGNATOR,
                'x1': consts.string.type.REAL,
                'y1': consts.string.type.REAL,
                'z1': consts.string.type.REAL,
                'ro': '0',
                'x2': consts.string.type.REAL,
                'y2': consts.string.type.REAL,
                'z2': consts.string.type.REAL,
                'f1': consts.string.type.REAL,
                'f2': consts.string.type.REAL,
                'f3': consts.string.type.REAL,
            },
            {
                'suffix': '5',
                'designator': None,
                'x1': consts.string.type.REAL,
                'y1': consts.string.type.REAL,
                'z1': consts.string.type.REAL,
                'ro': '0',
                'x2': consts.string.type.REAL,
                'y2': consts.string.type.REAL,
                'z2': consts.string.type.REAL,
                'f1': consts.string.type.REAL,
                'f2': consts.string.type.REAL,
                'f3': consts.string.type.REAL,
            },
            {
                'suffix': '5',
                'designator': consts.string.type.DESIGNATOR,
                'x1': None,
                'y1': consts.string.type.REAL,
                'z1': consts.string.type.REAL,
                'ro': '0',
                'x2': consts.string.type.REAL,
                'y2': consts.string.type.REAL,
                'z2': consts.string.type.REAL,
                'f1': consts.string.type.REAL,
                'f2': consts.string.type.REAL,
                'f3': consts.string.type.REAL,
            },
            {
                'suffix': '5',
                'designator': consts.string.type.DESIGNATOR,
                'x1': consts.string.type.REAL,
                'y1': None,
                'z1': consts.string.type.REAL,
                'ro': '0',
                'x2': consts.string.type.REAL,
                'y2': consts.string.type.REAL,
                'z2': consts.string.type.REAL,
                'f1': consts.string.type.REAL,
                'f2': consts.string.type.REAL,
                'f3': consts.string.type.REAL,
            },
            {
                'suffix': '5',
                'designator': consts.string.type.DESIGNATOR,
                'x1': consts.string.type.REAL,
                'y1': consts.string.type.REAL,
                'z1': None,
                'ro': '0',
                'x2': consts.string.type.REAL,
                'y2': consts.string.type.REAL,
                'z2': consts.string.type.REAL,
                'f1': consts.string.type.REAL,
                'f2': consts.string.type.REAL,
                'f3': consts.string.type.REAL,
            },
            {
                'suffix': '5',
                'designator': consts.string.type.DESIGNATOR,
                'x1': consts.string.type.REAL,
                'y1': consts.string.type.REAL,
                'z1': consts.string.type.REAL,
                'ro': None,
                'x2': consts.string.type.REAL,
                'y2': consts.string.type.REAL,
                'z2': consts.string.type.REAL,
                'f1': consts.string.type.REAL,
                'f2': consts.string.type.REAL,
                'f3': consts.string.type.REAL,
            },
            {
                'suffix': '5',
                'designator': consts.string.type.DESIGNATOR,
                'x1': consts.string.type.REAL,
                'y1': consts.string.type.REAL,
                'z1': consts.string.type.REAL,
                'ro': '0',
                'x2': None,
                'y2': consts.string.type.REAL,
                'z2': consts.string.type.REAL,
                'f1': consts.string.type.REAL,
                'f2': consts.string.type.REAL,
                'f3': consts.string.type.REAL,
            },
            {
                'suffix': '5',
                'designator': consts.string.type.DESIGNATOR,
                'x1': consts.string.type.REAL,
                'y1': consts.string.type.REAL,
                'z1': consts.string.type.REAL,
                'ro': '0',
                'x2': consts.string.type.REAL,
                'y2': None,
                'z2': consts.string.type.REAL,
                'f1': consts.string.type.REAL,
                'f2': consts.string.type.REAL,
                'f3': consts.string.type.REAL,
            },
            {
                'suffix': '5',
                'designator': consts.string.type.DESIGNATOR,
                'x1': consts.string.type.REAL,
                'y1': consts.string.type.REAL,
                'z1': consts.string.type.REAL,
                'ro': '0',
                'x2': consts.string.type.REAL,
                'y2': consts.string.type.REAL,
                'z2': None,
                'f1': consts.string.type.REAL,
                'f2': consts.string.type.REAL,
                'f3': consts.string.type.REAL,
            },
            {
                'suffix': '5',
                'designator': consts.string.type.DESIGNATOR,
                'x1': consts.string.type.REAL,
                'y1': consts.string.type.REAL,
                'z1': consts.string.type.REAL,
                'ro': '0',
                'x2': consts.string.type.REAL,
                'y2': consts.string.type.REAL,
                'z2': consts.string.type.REAL,
                'f1': None,
                'f2': consts.string.type.REAL,
                'f3': consts.string.type.REAL,
            },
            {
                'suffix': '5',
                'designator': consts.string.type.DESIGNATOR,
                'x1': consts.string.type.REAL,
                'y1': consts.string.type.REAL,
                'z1': consts.string.type.REAL,
                'ro': '0',
                'x2': consts.string.type.REAL,
                'y2': consts.string.type.REAL,
                'z2': consts.string.type.REAL,
                'f1': consts.string.type.REAL,
                'f2': None,
                'f3': consts.string.type.REAL,
            },
            {
                'suffix': '5',
                'designator': consts.string.type.DESIGNATOR,
                'x1': consts.string.type.REAL,
                'y1': consts.string.type.REAL,
                'z1': consts.string.type.REAL,
                'ro': '0',
                'x2': consts.string.type.REAL,
                'y2': consts.string.type.REAL,
                'z2': consts.string.type.REAL,
                'f1': consts.string.type.REAL,
                'f2': consts.string.type.REAL,
                'f3': None,
            },
            {
                'suffix': '1',
                'designator': consts.string.type.DESIGNATOR,
                'x1': consts.string.type.REAL,
                'y1': consts.string.type.REAL,
                'z1': consts.string.type.REAL,
                'ro': '0',
                'x2': consts.string.type.REAL,
                'y2': consts.string.type.REAL,
                'z2': consts.string.type.REAL,
                'f1': consts.string.type.REAL,
                'f2': consts.string.type.REAL,
                'f3': consts.string.type.REAL,
            },
            {
                'suffix': '5',
                'designator': consts.string.type.DESIGNATOR,
                'x1': consts.string.type.REAL,
                'y1': consts.string.type.REAL,
                'z1': consts.string.type.REAL,
                'ro': '3.1',
                'x2': consts.string.type.REAL,
                'y2': consts.string.type.REAL,
                'z2': consts.string.type.REAL,
                'f1': consts.string.type.REAL,
                'f2': consts.string.type.REAL,
                'f3': consts.string.type.REAL,
            },
        ]
