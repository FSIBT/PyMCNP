...
def fortran_integers(draw, condition=lambda _: True):
    ...
    return draw(st.integers().filter(condition))
...
def fortran_reals(draw, condition=lambda _: True):
    ...
    return draw(st.floats(allow_nan=False, allow_infinity=False).filter(condition))
...
def mcnp_cells(draw, valid_number, valid_material, valid_density, valid_geometry, valid_parameter):
    ...
    if valid_number is None:
        ...
        number = draw(fortran_integers(lambda i: i < 1))
...
def mcnp_parameter_importance(draw, valid_value: bool, valid_designator: bool):
    if valid_value is None:
        ...
        value = draw(fortran_integers(lambda i: i >= 0))
    else:
        value = draw(fortran_integers(lambda i: i < 0))

    designator = draw(mcnp_designators(valid_designator))

    return ("imp", None, designator, value, f"{value}")
...
def mcnp_parameter_volume(draw, valid_value: bool):
    if valid_value is None:
        ...
        value = draw(fortran_reals(lambda f: f > 0))
    else:
        value = draw(fortran_reals(lambda f: f <= 0))

    return ("vol", None, None, value, f"{value}")
...
def mcnp_parameter_protonWeight(draw, valid_value: bool):
    ...
    return ("pwt", None, None, value, f"{value}")
...
def mcnp_parameter_exponentialTransform(draw, valid_value: bool, valid_designator: bool):
    if valid_value is None:
        ...
        value = draw(fortran_integers())
    else:
        value = "a"

    designator = draw(mcnp_designators(valid_designator))

    return ("ext", None, designator, value, f"{value}")
...
def mcnp_parameter_forcedCollision(draw, valid_value: bool, valid_designator: bool):
    if valid_value is None:
        ...
        value = draw(fortran_integers(lambda i: -1 <= i <= 1))
    else:
        value = draw(fortran_integers(lambda i: -1 > i or i > 1))

    designator = draw(mcnp_designators(valid_designator))

    return ("fcl", None, designator, value, f"{value}")
...
def mcnp_parameter_weightWindowBounds(draw, valid_value: bool, valid_suffix: bool, valid_designator: bool):
    if valid_value is None:
        ...
        value = draw(fortran_integers(lambda i: i == -1 or i >= 0))
    else:
        value = draw(fortran_integers(lambda i: i != -1 and i < 0))

    if valid_suffix is None:
        suffix = None
    elif valid_suffix:
        suffix = draw(fortran_integers(lambda i: 1 <= i and i <= 99_999_999))
    else:
        suffix = draw(fortran_integers(lambda i: 1 > i or i > 99_999_999))

    designator = draw(mcnp_designators(valid_designator))

    return ("wwn", suffix, designator, value, f"{value}")
...
def mcnp_parameter_dxtranContribution(draw, valid_value: bool, valid_suffix: bool, valid_designator: bool):
    if valid_value is None:
        ...
        value = draw(fortran_integers(lambda i: 0 <= i <= 1))
    else:
        value = draw(fortran_integers(lambda i: 0 > i or 1 < i))
    ...
    return ("dxc", suffix, designator, value, f"{value}")
...
def mcnp_parameter_fissionTurnOff(draw, valid_value: bool):
    if valid_value is None:
        ...
        value = draw(fortran_integers(lambda i: i not in {0, 1, 2}))

    return ("nonu", None, None, value, f"{value}")
...
def mcnp_parameter_detectorContribution(draw, valid_value: bool, valid_suffix: bool):
    ...
    return ("pd", suffix, None, value, f"{value}")
...
def mcnp_parameter_gasThermalTemperature(draw, valid_value: bool, valid_suffix: bool):
    if valid_value is None:
        ...
        value = draw(fortran_reals(lambda i: i >= 0))
    else:
        value = draw(fortran_reals(lambda i: i < 0))
    ...
    return ("tmp", suffix, None, value, f"{value}")
...
def mcnp_parameter_universe(draw, valid_value: bool):
    if valid_value is None:
        ...
        value = draw(fortran_integers(lambda i: 1 <= i <= 99_999_999))
    else:
        value = draw(fortran_integers(lambda i: 1 > i or i > 99_999_999))

    return ("u", None, None, value, f"{value}")
...
def mcnp_parameter_coordinateTransformation_form1(draw, valid_value: bool):
    ...
    return ("trcl", None, None, value, f"{value}")
...
def mcnp_parameter_coordinateTransformation_form2(draw, valid_value: bool):
    if valid_value is None:
        ...
        value = tuple([draw(fortran_integers())] * 12 + [draw(st.sampled_from([-1, 1]))])
    else:
        value = "a"

    return ("trcl", None, None, value, " ".join(str(entry) for entry in value))
...
def mcnp_parameter_lattice(draw, valid_value: bool):
    if valid_value is None:
        ...
        value = draw(fortran_integers(lambda i: i not in {1, 2}))

    return ("lat", None, None, value, f"{value}")
...
def mcnp_parameter_fill_form1A(draw, valid_value: bool):
    if valid_value is None:
        ...
        value = (draw(fortran_integers(lambda i: 0 <= i <= 99_999_999)), None)
    else:
        value = (draw(fortran_integers(lambda i: 0 > i or i > 99_999_999)), None)

    return ("fill", None, None, value, f"{value[0]} {value[1]}")
...
def mcnp_parameter_fill_form1B(draw, valid_value: bool):
    if valid_value is None:
        ...
        value = (
            ...
            draw(fortran_integers(lambda i: 1 <= i <= 999)),
        )
    else:
        value = (
            draw(fortran_integers(lambda i: 0 > i or i > 99_999_999)),
            draw(fortran_integers(lambda i: i > 999)),
        )
...
def mcnp_parameter_fill_form2(draw, valid_value: bool):
    if valid_value is None:
        ...
        value = (
            ...
            (j_lower, j_upper),
            (k_lower, k_upper),
            tuple([draw(fortran_integers(lambda i: 0 <= i <= 99_999_999))] * count),
        )
    else:
        value = (
            (2, 1),
    ...
    return (
        ...
        None,
        value,
        f"{value[0][0]}:{value[0][1]} {value[1][0]}:{value[1][1]} {value[2][0]}:{value[2][1]} "
        + " ".join([str(entry) for entry in value[3]]),
    )
...
def mcnp_parameter_energyCutoff(draw, valid_value: bool, valid_designator: bool):
    ...
    return ("elpt", None, designator, value, f"{value}")
...
def mcnp_parameter_cosy(draw, valid_value: bool):
    ...
    return ("cosy", None, None, value, f"{value}")
...
def mcnp_parameter_bfield(draw, valid_value: bool):
    ...
    return ("bflcl", None, None, value, f"{value}")
...
def mcnp_parameter_uncolidedSecondaries(draw, valid_value: bool, valid_designator: bool):
    if valid_value is None:
        ...
        value = draw(fortran_integers(lambda i: i not in {0, 1}))

    designator = draw(mcnp_designators(valid_designator))

    return ("unc", None, designator, value, f"{value}")
...
def mcnp_surface_ellipsoids(draw, valid_number, valid_transform, valid_mnemonic):
    ...
    entries = [
        ...
        draw(fortran_reals()),
        draw(fortran_reals()),
        draw(fortran_reals(lambda r: r != 0)),
    ]
