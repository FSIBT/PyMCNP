...
class TestSurface:
	...
	class TestfromMcnp:
		...
		@hy.settings(max_examples = HY_TRIALS)
		@hy.given(surface = mcnp_surfaces(), number = fortran_integers(lambda i: i < 1 or i > 99_999_999))
		...
		@hy.settings(max_examples = HY_TRIALS)
		@hy.given(surface = mcnp_surfaces(), mnemonic = st.from_regex(r"\A[a-z]+\Z").filter(lambda s: s not in SURFACE_MNEMONICS))
