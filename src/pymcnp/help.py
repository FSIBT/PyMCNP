...
def generate_cellparameters():
	...
	out = ''
	for className, classComment, valueName, valueComment, valueCheck, has_designator, has_suffix in DATA:
		...
		out += f"\t\t\"\"\"\n"
		out += f"\n"
		out += f"\t\tif value is None or not ({valueCheck}):\n" if valueCheck else f"\t\tif value is None:\n"
		...
		if has_designator:
			...
			out += f"\t\n"
			out += f"\t\tParameters:\n"
			out += f"\t\t\tdesignator (Designator): INP cell card parameter designator.\n"
			out += f"\t\t\"\"\"\n"
			out += f"\t\n"
