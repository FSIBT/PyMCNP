"""
Contains the ``Embdf`` subclass of ``Data``.
"""
...
class Embdf(Data):
    """
    Represents INP embdf data cards.

    ``Embdf`` implements ``Data``.
    ...
    def __init__(self, multipliers: tuple[types.McnpReal], suffix: types.McnpInteger):
        ...
        self.mnemonic: Final[DataMnemonic] = DataMnemonic.EMBDF
        ...
        self.ident: Final[str] = f'embdf{self.suffix}'

    @staticmethod
    def from_mcnp(source: str):
        """
        Generates ``Embdf`` objects from INP.

        ``from_mcnp`` translates from INP to PyMCNP; it parses INP.

        Parameters:
            source: INP for embdf data cards.
    ...
    def to_mcnp(self) -> str:
        """
        Generates INP from ``Embdf`` objects.
