"""
Contains the ``Embtm`` subclass of ``Data``.
"""
...
class Embtm(Data):
    """
    Represents INP embtm data cards.

    ``Embtm`` implements ``Data``.
    ...
    def __init__(self, multipliers: tuple[types.McnpReal], suffix: types.McnpInteger):
        ...
        self.mnemonic: Final[DataMnemonic] = DataMnemonic.EMBTM
        ...
        self.ident: Final[str] = f'embtm{self.suffix}'

    @staticmethod
    def from_mcnp(source: str):
        """
        Generates ``Embtm`` objects from INP.

        ``from_mcnp`` translates from INP to PyMCNP; it parses INP.

        Parameters:
            source: INP for embtm data cards.
    ...
    def to_mcnp(self) -> str:
        """
        Generates INP from ``Embtm`` objects.
