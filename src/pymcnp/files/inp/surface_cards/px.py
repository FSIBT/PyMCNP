"""
Contains the ``Px`` subclass of ``Surface``.
"""
...
class Px(Surface):
    """
    Represents INP px surface cards.

    ``Px`` implements ``Surface``.
    ...
    def __init__(
        ...
        self.mnemonic: Final[SurfaceMnemonic] = SurfaceMnemonic.PX
    ...
    def to_pyvista(self):
        """
        Generates ``pyvista.PolyData`` representing ``Px``.

        Returns:
            ``pyvista.PolyData`` for ``Px``.
        """

        vis = _visualization.PyMcnpVisualization.get_plane(1, 0, 0, self.d.value)
