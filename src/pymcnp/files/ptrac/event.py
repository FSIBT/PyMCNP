...
class Event:
	...
	@classmethod
	def from_mcnp(cls, source: str, header: Header, event_type: EventTypes) -> tuple[Self, str]:
		"""
		'from_mcnp'
		"""
		...
		match event_type:
			...
			case cls.EventTypes.TERMINAL:
				# (NTER: 14)
				value = EventNter.cast_mcnp_nter(j_line.popl())
	...
	def to_arguments(self) -> dict:
		...
		return {'type': self.type, 'node': self.node, 'nsr': self.nsr, 'nxs': self.nxs, 'ntyn_mtp': self.ntyn_mtp, 'nsf': self.nsf, 'surface_angle': self.surface_angle, 'nter': self.nter, 'branch': self.branch, 'ipt': self.ipt, 'ncl': self.ncl, 'mat': self.mat, 'ncp': self.ncp, 'xxx': self.xxx, 'yyy': self.yyy, 'zzz': self.zzz, 'uuu': self.uuu, 'vvv': self.vvv, 'www': self.www, 'erg': self.erg, 'wgt': self.wgt, 'tme': self.tme}
