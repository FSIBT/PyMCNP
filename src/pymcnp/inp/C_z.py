...
class C_z(_card.Card):
    ...
    _REGEX = re.compile(rf'\A(\+|\*)?(\S+)( \S+)? c/z( {types.Real._REGEX.pattern[2:-2]})( {types.Real._REGEX.pattern[2:-2]})( {types.Real._REGEX.pattern[2:-2]})\Z', re.IGNORECASE)
    ...
    def to_mcnp(self):
        """
        Generates INP from `C_z`.

        Returns:
            INP surface card.
        """

        source = f'{self.prefix if self.prefix is not None else ""}{self.number} {self.transform if self.transform is not None else ""} {self._KEYWORD} {self.x} {self.y} {self.r}'
