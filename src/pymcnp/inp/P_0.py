...
class P_0(_card.Card):
    """
    Represents INP `p` surface cards variation #0.
    """
    ...
    _REGEX = re.compile(
        rf'\A(\+|\*)?(\S+)( \S+)? p( {types.Real._REGEX.pattern[2:-2]})( {types.Real._REGEX.pattern[2:-2]})( {types.Real._REGEX.pattern[2:-2]})( {types.Real._REGEX.pattern[2:-2]})\Z', re.IGNORECASE
    )
    ...
    def to_mcnp(self):
        """
        Generates INP from `P_0`.

        Returns:
            INP surface card.
        """

        source = f'{self.prefix if self.prefix is not None else ""}{self.number} {self.transform if self.transform is not None else ""} {self._KEYWORD} {self.a} {self.b} {self.c} {self.d}'
