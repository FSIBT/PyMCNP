...
class Sz(_card.Card):
    ...
    _REGEX = re.compile(rf'\A(\+|\*)?(\S+)( \S+)? sz( {types.Real._REGEX.pattern[2:-2]})( {types.Real._REGEX.pattern[2:-2]})\Z', re.IGNORECASE)
    ...
    def to_mcnp(self):
        """
        Generates INP from `Sz`.

        Returns:
            INP surface card.
        """

        source = f'{self.prefix if self.prefix is not None else ""}{self.number} {self.transform if self.transform is not None else ""} {self._KEYWORD} {self.z} {self.r}'
