...
class MEntry_Substance(_entry.MEntry_):
    ...
    def __init__(self, zaid: types.Zaid, fraction: types.Real):
        ...
        if zaid is None:
            raise errors.InpError(errors.InpCode.SEMANTICS_DATA_OPTION_ENTRY_VALUE, zaid)
        if fraction is None or not (-1 <= fraction <= 1):
            raise errors.InpError(errors.InpCode.SEMANTICS_DATA_OPTION_ENTRY_VALUE, fraction)
    ...
    def from_mcnp(source: str):
        ...
        if not tokens:
            raise errors.InpError(errors.InpCode.SYNTAX_M_ENTRY, source)
