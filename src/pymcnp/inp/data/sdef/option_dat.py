...
class SdefOption_Dat(_option.SdefOption_, keyword='dat'):
    ...
    def __init__(self, month: types.Integer, day: types.Integer, year: types.Integer):
        ...
        if month is None or not (1 <= month <= 12):
            raise errors.McnpError(errors.McnpCode.SEMANTICS_DATA_OPTION_VALUE, month)
        if day is None or not (1 <= day <= 31):
            raise errors.McnpError(errors.McnpCode.SEMANTICS_DATA_OPTION_VALUE, day)
        if year is None or not (1 <= year <= 9999):
            raise errors.McnpError(errors.McnpCode.SEMANTICS_DATA_OPTION_VALUE, year)
