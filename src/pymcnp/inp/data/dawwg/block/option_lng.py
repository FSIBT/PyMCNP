...
class BlockOption_Lng(_option.BlockOption_, keyword='lng'):
    """
    Represents INP data card data option dawwg option lng options.

    Attributes:
        setting: Number of the last neutron group.
    """

    _REGEX = re.compile(r'\Alng( \S+)\Z')

    def __init__(self, setting: types.Integer):
        """
        Initializes ``BlockOption_Lng``.
    ...
    def from_mcnp(source: str):
        """
        Generates ``BlockOption_Lng`` from INP.
        ...
        tokens = BlockOption_Lng._REGEX.match(source)
        ...
        return BlockOption_Lng(setting)
