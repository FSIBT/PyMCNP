...
class Ylim(MplotOption, keyword='ylim'):
    """
    Represents INP ylim elements.
    ...
    _REGEX = re.compile(
        rf'\Aylim( {types.Real._REGEX.pattern})( {types.Real._REGEX.pattern})( {types.Real._REGEX.pattern})\Z'
    )
...
class YlimBuilder:
    ...
    def build(self):
        """
        Builds ``YlimBuilder`` into ``Ylim``.

        Returns:
            ``Ylim`` for ``YlimBuilder``.
        """
