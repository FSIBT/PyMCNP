...
class Kx(_card.Card):
    """
    Represents INP `kx` surface cards.
    """
    ...
    _REGEX = re.compile(rf'\A(\+|\*)?(\S+)( \S+)? kx( {types.Real._REGEX.pattern[2:-2]})( {types.Real._REGEX.pattern[2:-2]})( {types.Real._REGEX.pattern[2:-2]})\Z', re.IGNORECASE)
    ...
    def to_mcnp(self):
        """
        Generates INP from `Kx`.

        Returns:
            INP surface card.
        """

        source = f'{self.prefix if self.prefix is not None else ""}{self.number} {self.transform if self.transform is not None else ""} {self._KEYWORD} {self.x} {self.t_squared} {self.plusminus_1}'
