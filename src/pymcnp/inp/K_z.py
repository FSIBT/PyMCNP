...
class K_z(_card.Card):
    ...
    _REGEX = re.compile(
        rf'\A(\+|\*)?(\S+)( \S+)? k/z( {types.Real._REGEX.pattern[2:-2]})( {types.Real._REGEX.pattern[2:-2]})( {types.Real._REGEX.pattern[2:-2]})( {types.Real._REGEX.pattern[2:-2]})( {types.Real._REGEX.pattern[2:-2]})\Z',
        re.IGNORECASE,
    )
    ...
    def y(self) -> types.Real:
        """
        Parallel-to-z-axis cone center y component
    ...
    def z(self) -> types.Real:
        """
        Parallel-to-z-axis cone center z component
