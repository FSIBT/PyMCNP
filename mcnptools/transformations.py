"""
Created on Tue Dec 29 12:19:34 2020
...
class Transformation:
    ...
    def make_transformation(self, alpha, beta, gamma, trans=[0,0,0]):
        ...
        x2, y2, z2 = xyz2[:,0] + M_trans[0], xyz2[:,1] + \
            M_trans[1], xyz2[:,2] + M_trans[2]
    ...
    def plot_surf(self, ax_3d, color="black", soil=False, plot_axes=False):
        ...
        if soil: #  add end caps to the cylinder
            ax_3d.plot_surface(self.xt, self.yt, self.zt,
                           color="blue", alpha=1)
    ...
    def plot_axes(self, ax_3d, ax_length=None):
        if ax_length is None:
            ax_length = max(abs(self.x.max()), abs(self.y.max()),
                            abs(self.z.max()))
            
        ax_3d.quiver(0, 0, 0, ax_length+5, 0, 0, color="red", alpha=0.5,
                     arrow_length_ratio=0.05)
        ax_3d.quiver(0, 0, 0, 0, ax_length+5, 0, color="green", alpha=0.5,
                     arrow_length_ratio=0.05)
        ax_3d.quiver(0, 0, 0, 0, 0, ax_length+5, color="blue", alpha=0.5,
                     arrow_length_ratio=0.05)
