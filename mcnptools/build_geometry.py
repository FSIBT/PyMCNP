...
class Geometry:
    def __init__(
        self, color, name, h_menu, soil=False, fig=None, ax_3d=None, ax_menu=None
    ):
        # initialize figure
        self.color = color
        self.name = name
        ...
        if soil == False:
            ...
            # X-rotation
            # h_menu = 0.89
            self.ax_rotx = plt.axes([0.77, h_menu, 0.2, 0.03], facecolor=axcolor)
            self.s_rotx = Slider(
                self.ax_rotx, "Rotation-x", 0, 360, valinit=0, valfmt="%1d"
            )
            self.ax_rotx.set_title(name, color=self.color)
            # Y-rotation
            self.ax_roty = plt.axes([0.77, h_menu - 0.05, 0.2, 0.03], facecolor=axcolor)
            self.s_roty = Slider(
                self.ax_roty, "Rotation-y", 0, 360, valinit=0, valfmt="%1d"
            )
            # Z-rotation
            self.ax_rotz = plt.axes(
                [0.77, h_menu - 2 * 0.05, 0.2, 0.03], facecolor=axcolor
            )
            self.s_rotz = Slider(
                self.ax_rotz, "Rotation-z", 0, 360, valinit=0, valfmt="%1d"
            )

            ## translation text boxes
            self.ax_box1 = plt.axes([0.77, h_menu - 3 * 0.05, 0.04, 0.03])
            self.text_box1 = TextBox(
                self.ax_box1, "X-tr", initial="0", color="lightblue"
            )

            self.ax_box2 = plt.axes([0.85, h_menu - 3 * 0.05, 0.04, 0.03])
            self.text_box2 = TextBox(
                self.ax_box2, "Y-tr", initial="0", color="lightblue"
            )

            self.ax_box3 = plt.axes([0.93, h_menu - 3 * 0.05, 0.04, 0.03])
            self.text_box3 = TextBox(
                self.ax_box3, "Z-tr", initial="0", color="lightblue"
            )

        else:
            # write to MCNP
            self.ax_bmcnp = plt.axes([0.88, 0.05, 0.1, 0.05], facecolor="lightgreen")
            self.bmcnp = Button(
                self.ax_bmcnp,
                "write to MCNP",
                color="lightgreen",
    ...
    def make_object(self, x0, y0, z0, height, radius, axis):
        ...
        self.object.make_cylinder(
            x0=x0, y0=y0, z0=z0, height=height, radius=radius, axis=axis
        )
        if self.soil:
            self.object.plot_surf(
                self.ax_3d, color=self.color, soil=True, plot_axes=True
            )
        else:
    ...
    def update(self, val):
        for g in self.geom_lst:
            if g.soil == False:
                ...
                g.object.make_transformation(
                    alpha=xval, beta=yval, gamma=zval, trans=trans
                )

        self.keep_zoom()
        for g in self.geom_lst:
            if g.soil:
                g.object.plot_surf(g.ax_3d, color=g.color, soil=True, plot_axes=True)
            else:
